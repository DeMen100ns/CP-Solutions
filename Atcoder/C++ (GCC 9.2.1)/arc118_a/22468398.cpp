...
signed main(){
    ...
    cin >> n;
    int cd=n;
  //  For(i,5,5000) if (checkprime(i)) pr.pb(i);
    int cnt=0, j=0;
    For(i,1,min(1000,n-2)) ans.insert(10*i), cnt++;
    while(ans.size()<=n-2) {j++; if(j*6>10000) break; ans.insert(6*j);}
    j=0; while(ans.size()<=n-2) {j++; ans.insert(14*j);}
    ans.insert(3*5*7);
    fora(i,ans) cout << i << " ";
    return 0;
}
