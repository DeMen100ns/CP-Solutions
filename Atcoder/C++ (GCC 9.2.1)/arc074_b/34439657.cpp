...
Name: 3N Numbers
Sauce: https://atcoder.jp/contests/abc062/tasks/arc074_b
Tag: Greedy
...
void solve()
{
    ...
    for(int i = 1; i <= 3 * n; ++i) a[i] *= -1;

    cal(suf);

    reverse(suf + 1, suf + 3 * n + 1);

    for(int i = 1; i <= 3 * n; ++i) suf[i] *= -1;

    int ma = -INF;
    for(int i = n; i <= 2 * n; ++i){
        ma = max(ma, pre[i] - suf[i + 1]);
    }
