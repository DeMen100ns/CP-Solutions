        ...
        For(lower,0,1){
            For(sum,0,d-1){
                For(positive,0,1){
                    For(digit,0,9){
                        int n=k[i-1]-'0';
                        if (!lower && digit>n) continue;
                        bool lower2=lower|(digit<n);
                        bool positive2=positive|digit;
                        int sum2=(sum+digit)%d;
                        dp[i][lower2][sum2][positive2]+=
                        dp[i-1][lower][sum][positive];
                        dp[i][lower2][sum2][positive2]%=MOD;
                    }
                }
            }
        }
    }
    return dp[len][0][0][1]+dp[len][1][0][1];
}
