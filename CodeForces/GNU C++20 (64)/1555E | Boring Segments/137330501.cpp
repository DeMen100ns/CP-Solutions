...
void upd(ll id, ll l, ll r, ll u, ll v, ll val) {
    ...
    if (u <= l && r <= v) {
        f[id]+=val;
        if (f[id]==1) seg[id]=(r-l+1);
        if (!f[id]){
            if (l<r) seg[id]=seg[id<<1]+seg[id<<1|1];
            else seg[id]=0;
        }
        return;
    }
    ...
    if (!f[id] && l<r) seg[id] = (seg[id<< 1] + seg[id<< 1 | 1]);
}
...
signed main(){
    ...
    For(i,1,n){
        while (j<=n && !f){
            upd(1,1,m,a[j].y.x,a[j].y.y,1);
            j++;
            if (seg[1]==m-1) f=true;
        }
        //cout << i << " " << j << " " << seg[1] << endl;
        if (seg[1]==m-1) mi=min(mi,a[j-1].x-a[i].x);
        //cout << a[i].x << endl;
        upd(1,1,m,a[i].y.x,a[i].y.y,-1);
        if (seg[1]<m-1) f=false;
    }
    cout << mi;
    return 0;
}
