...
string s,t;
int pw[N], hashs[N], hasht, f[N], dp[N][3];
 
int gethash(int l, int r){
    l--;
    return (hashs[r] - hashs[l] * pw[r - l] + MOD * MOD) % MOD;
}
 
void setup(int n, int m){
    pw[0] = 1;
    For(i,1,1e5 + 1) pw[i] = mul(pw[i - 1], base);
    For(i,1,n){
        hashs[i] = mul(hashs[i - 1], base);
        hashs[i] = add(hashs[i], s[i] - 'a' + 1);
    }
    For(i,1,m){
        hasht = mul(hasht, base);
        hasht = add(hasht, t[i] - 'a' + 1);
    }
    For(i,1,n - m + 1){
        f[i + m - 1] = (gethash(i,i + m - 1) == hasht);
    }
    //write(f,1,n);
}
...
void solve(){
    ...
    cin >> t; int m = t.size(); t = " " + t;
    setup(n,m);
    For(i,1,n){
        if (!f[i]) dp[i][0] = dp[i - 1][0];
        else {
            dp[i][0] = add(dp[i - m][2], i - m + 1);
        }
        dp[i][1] = add(dp[i - 1][1], dp[i][0]);
        dp[i][2] = add(dp[i - 1][2], dp[i][1]);
    }
    cout << dp[n][1];
}
