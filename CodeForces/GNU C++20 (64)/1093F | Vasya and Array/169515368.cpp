...
int dp[N][105], cnt[N][105], Sum[N], a[N];
...
void solve()
{
    ...
    for(int i = 2; i <= n + 1; ++i){
        ...
        for(int j = 1; j <= k; ++j){
            ...
            if (i < len || (cnt[i][j] + cnt[i][0]) - (cnt[i - len][j] + cnt[i - len][0]) < len) continue;
            ...
            add(dp[i][j], dp[i - len][j] - Sum[i - len]);
        }
    }
