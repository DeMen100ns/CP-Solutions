...
void solve(){
    ...
    For(i,1,9){
        ...
        if (cnt[i] && cnt2[10 - i]){
            ...
            cnt[i]--;
            cnt2[10 - i]--;
            For(j,0,9) n0 += min(cnt[j], cnt2[9 - j]);
            cnt[i]++;
            cnt2[10 - i]++;
        }
        if (n0 > ma){
    ...
    For(i,0,9){
        For(k,1,min(cnt[i],cnt2[9 - i])){
            p1 += (i + '0');
            p2 += (9 - i + '0');
        }
    }
    For(i,0,9){
        int mi = min(cnt[i], cnt2[9 - i]);
        cnt[i] -= mi;
        cnt2[9 - i] -= mi;
    }
    For(i,1,min(cnt[0], cnt2[0])){
        p1 = '0' + p1;
        p2 = '0' + p2;
    }
    int mi = min(cnt[0], cnt2[0]);
    cnt[0] -= mi;
    cnt2[0] -= mi;
    For(i,0,9){
        For(k,1,cnt[i]){
