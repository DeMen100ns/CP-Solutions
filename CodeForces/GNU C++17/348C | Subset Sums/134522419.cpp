...
int cnt_intersec(int i, int j){
    int cnt = 0;
    fora(k,b[i]){
        cnt += (binary_search(b[j].begin(), b[j].end(), k));
    }
    return cnt;
}
...
void solve(){
    ...
    For(i,1,n){
        For(j,0,(int)heavy.size() - 1){
            inter[i][j] = cnt_intersec(i,heavy[j]);
           // cout << inter[i][j] << " ";
        } //cout << endl;
    }
    while (q--){
        ...
        if (type == '+'){
            cin >> pos >> val;
            if (b[pos].size() < B){
                fora(i,b[pos]) a[i] += val;
                For(j,0,(int)heavy.size() - 1){
                    S[heavy[j]] += (ll)inter[pos][j] * val;
                }
            } else {
                ans[pos] += val;
            }
            ...
            cin >> pos;
            if (b[pos].size() < B){
                fora(i, b[pos]) sum += a[i];
            } else {
                sum = S[pos]; 
            }
            For(j,0,(int)heavy.size() - 1){
                sum += (ll)inter[pos][j] * ans[heavy[j]];
            }
            cout << sum << endl;
        }
    }
